...
class SinglePlayer:
    def __init__(self, root,language=None):
        ...
        player1_label = tk.Label(self.app, text=player1_text, font=('Helvetica', 18, 'bold'), bg=bg_color, fg='black')
        player1_label.pack(pady=10)

        if self.current_language == 'tr':
            select_character_text = "Karakterinizi Seçiniz"
        else:
            select_character_text = "Select Character"

        select_character_label= tk.Label(self.app, text=select_character_text, font=('Helvetica', 15, 'bold'), bg=bg_color, fg='black')
        select_character_label.place(x=10, y=120)

        x_button = tk.Button(self.app, text="X", command=self.select_x, font=('Helvetica', 12, 'bold'), bg='gray', fg='black', height=2, width=4)
        x_button.place(x=220,y=105)

        o_button = tk.Button(self.app, text="O", command=self.select_o, font=('Helvetica', 12, 'bold'), bg='gray', fg='black', height=2, width=4)
        o_button.place(x=300,y=105)
        ...
        three_button.place(x=140,y=300)
        ...
        eleven_button.place(x=360,y=300)
        ...
        play_button = tk.Button(self.app, text=play_text, command=self.select_play, font=('Helvetica', 12, 'bold'), bg='gray', fg='black')
        play_button.place(x=140,y=440, width=150, height=50)
    ...
    def select_settings_button(self):
        ...
        subprocess.run(["python", "settings_single.py"])
        self.play_sound()
    ...
    def select_play(self):
        ...
        if self.starting_symbol is None:
            ...
            if self.total_rounds == 1:
                ...
                tur_files = "stur_11.py"
            else:
                messagebox.showerror("Error", "Invalid number of rounds.")
                return

            tur_paths = os.path.join(os.path.dirname(__file__), tur_files)
            subprocess.run(["python", tur_paths, str(self.starting_symbol), str(self.total_rounds)], shell=True)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        language = sys.argv[1]
    app = SinglePlayer(root, language)
    root.mainloop()
